...
// Public Careers / Job Application
(function(){
  const form=document.getElementById("jobApplicationForm"); if(!form)return;
  form.addEventListener("submit",async function(e){e.preventDefault();
    const msgEl=document.getElementById("jobApplicationMsg");
    const client=sb;
    if(!client){msgEl.textContent=currentLanguage === "hi" ? "आवेदन सेवा अभी उपलब्ध नहीं है।" : "Application service is not configured.";return;}
    const photoFile=document.getElementById("ja_photo_file")?.files?.[0]||null;
    const resumeFile=document.getElementById("ja_resume_file")?.files?.[0]||null;
    msgEl.textContent=currentLanguage === "hi" ? "फाइलें अपलोड हो रही हैं…" : "Uploading files…";
    async function uploadCareerFile(file,folder){
      if(!file)return "";
    ...
    try{ photoUrl=await uploadCareerFile(photoFile,"job-applications/photos"); resumeUrl=await uploadCareerFile(resumeFile,"job-applications/resumes"); }
    catch(err){msgEl.textContent=(currentLanguage === "hi" ? "फाइल अपलोड नहीं हो सकी: " : "File upload failed: ")+err.message;return;}
    const obj={full_name:document.getElementById("ja_name").value.trim(),mobile:document.getElementById("ja_mobile").value.trim(),email:document.getElementById("ja_email").value.trim(),applied_post:document.getElementById("ja_post").value,qualification:document.getElementById("ja_qualification").value.trim(),experience:document.getElementById("ja_experience").value.trim(),address:document.getElementById("ja_address").value.trim(),photo_url:photoUrl,resume_url:resumeUrl,message:document.getElementById("ja_message").value.trim(),status:"new"};
    if(!obj.full_name||!obj.mobile||!obj.applied_post||!obj.address){msgEl.textContent=currentLanguage === "hi" ? "कृपया सभी आवश्यक जानकारी भरें।" : "Please fill all required fields.";return;}
    const r=await client.from("job_applications").insert(obj);
    if(r.error){msgEl.textContent=(currentLanguage === "hi" ? "आवेदन जमा नहीं हो सका: " : "Application could not be submitted: ")+r.error.message;return;}
    form.reset();msgEl.textContent=currentLanguage === "hi" ? "आवेदन सफलतापूर्वक जमा हो गया। समीक्षा के बाद SS Enterprises आपसे संपर्क करेगा।" : "Application submitted successfully. SS Enterprises will contact you after review.";
  });
})();

try{ if(typeof translateCareerForm === "function") translateCareerForm(); }catch(e){}
