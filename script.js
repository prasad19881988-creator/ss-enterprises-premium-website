const DEFAULT_DATA={
 ...
 team:[
 {id:"founder",role:"Founder",name:"Add Name",location:"Darbhanga, Bihar",responsibilities:"Overall vision, strategic decisions and major business operations.",photo:""},
 {id:"ceo",role:"CEO & Managing Director",name:"Add Name",location:"Darbhanga, Bihar",responsibilities:"Day-to-day operations, projects/tenders, team management and organisational growth.",photo:""},
 {id:"state-head",role:"State Head",name:"Add Name",location:"Bihar",responsibilities:"State-level project coordination, field operations and monitoring of district teams.",photo:""},
 {id:"district-coordinator",role:"District Coordinator",name:"Add Name",location:"Darbhanga, Bihar",responsibilities:"District project implementation, field staff coordination and monitoring of assigned work.",photo:""}]};
...
function renderTeam(){
 ...
 grid.innerHTML=data.team.map(t=>`<article class="person-card">${t.photo?`<img src="${t.photo}" alt="${escapeHtml(t.name)}">`:`<div class="person-placeholder">♙</div>`}<div><span class="role">${escapeHtml(t.role)}</span><h3>${escapeHtml(t.name)}</h3><p>📍 ${escapeHtml(t.location)}</p><p>${escapeHtml(t.responsibilities)}</p></div></article>`).join("");
}
...
async function loadData(){
 ...
 if(!error&&row?.content)data=row.content;
