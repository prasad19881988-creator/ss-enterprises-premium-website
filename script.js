...
(function(){
  ...
  form.addEventListener("submit",async function(e){e.preventDefault();
    ...
    const client=sb;
    if(!client){msgEl.textContent="Application service is not configured.";return;}
    ...
    if(!obj.full_name||!obj.mobile||!obj.applied_post||!obj.address){msgEl.textContent="Please fill all required fields.";return;}
    const r=await client.from("job_applications").insert(obj);
    if(r.error){msgEl.textContent="Application could not be submitted: "+r.error.message;return;}
    form.reset();msgEl.textContent="Application submitted successfully. SS Enterprises will contact you after review.";
  });
})();
