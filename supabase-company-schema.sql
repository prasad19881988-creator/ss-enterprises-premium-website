...
create table if not exists public.staff (
  ...
  employee_signature_url text default '',
...
create table if not exists public.letters (
  id uuid primary key default gen_random_uuid(),
  staff_id uuid not null references public.staff(id) on delete cascade,
  letter_type text not null check (letter_type in ('offer','joining','warning','termination')),
...
-- Safe additive upgrade for existing installations.
...
-- HR LETTERS MIGRATION (run this in Supabase SQL Editor if the existing letters table was already created)
